...
## Overview
`PBS/*.txt` are the human-editable source of truth for game data; `Scripts/021_Compiler/002_Compiler_CompilePBS.rb` parses them into binary `Data/*.dat`, loaded into `GameData::*` classes at runtime (see `source-code-audit-and-refactoring`). Never hand-edit `Data/*.dat` — it's regenerated from PBS on next boot whenever a PBS file's mtime is newer, and any manual edit there is silently discarded. For pipeline-level failures (plugin load, versioning, no-CI) rather than a single bad field, see `build-system-audit-and-refactoring` instead.
...
- Must be saved as **UTF-8**. A wrong-encoding save produces the same "Expected a section" error as a missing header, which is misleading — if that error appears on a file just edited in a non-UTF-8-aware editor, check encoding first.
...
4. To confirm the compiled data matches intent, the compiler's inverse (`003_Compiler_WritePBS.rb`) is what in-game data editors (F9) use to write PBS back out — editing a record via the F9 editor and diffing the resulting PBS line against what was written by hand is a good way to catch subtle format mismatches (field order, quoting) that don't error but silently produce wrong values.
5. See `verifying-essentials-changes` for the general in-engine verification workflow this feeds into.
