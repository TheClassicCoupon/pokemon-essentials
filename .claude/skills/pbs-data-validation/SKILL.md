...
`Data/Scripts/021_Compiler/001_Compiler.rb` has typed field coercers (`csvInt`, `csvPosInt`, `csvFloat`, `csvBoolean`, `csvEnumField`, etc.) that each PBS schema field is declared against. Validation failures always append `FileLineData.linereport` to the error — this gives you the exact file and line number, so the fastest triage is: read the error, jump to that line, check the field's declared type in the relevant schema (search `SCHEMA` in the class's `GameData::*` file under `010_Data`).
...
- Copy-pasting a block between species/moves/items and forgetting to update the `[InternalName]` header — causes silent duplicate-ID overwrites (`.register` keys by both `:id` and `:id_number`) rather than a compile error.
