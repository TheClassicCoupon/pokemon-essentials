...
Pokémon Essentials v21.1 — a Ruby-based engine/framework for building Pokémon fangames, originally built for RPG Maker XP (RGSS) and run today via the mkxp-z runtime (`Game.exe` on Windows, configured by `mkxp.json`/`Game.ini`). This repo is the engine itself, intended to be forked and built on top of (see README.md); there is no separate "app" layer.

There is no npm/cargo/make-style build system, no automated test suite, and no CI/CD in this repo (no `.github/workflows`) — it's a Ruby scripting layer loaded into a game engine at runtime. "Building" means launching the game, which triggers an in-engine compile step (see below). Verification is manual, in-engine — see the `verifying-essentials-changes` skill.
...
016_UI                  Menu/screen implementations (Pokedex, Bag, Summary, PC, Mart, etc.); 001_NonInteractiveUI subfolder
017_Minigames
...
019_Utilities
020_Debug               Debug menus, in-game data editors (F9): 001_EditorScreens, 002_AnimationEditor, 003_Debug menus
...
**Folder-naming note:** most subfolders under the numbered folders have had spaces stripped from their names in recent restructuring passes (e.g. `020_Debug/001_EditorScreens`), but this isn't complete yet — some still have spaces (`020_Debug/003_Debug menus`, `010_Data/001_Hardcoded data`, `010_Data/002_PBS data`). Don't assume a space-free name for a subfolder you haven't confirmed with `ls`/Glob; the repo is mid-restructure and this is expected to keep changing (see the `claude-repository-adaptation` skill).

## Data pipeline: PBS ↔ GameData

`PBS/` contains human-editable, plain-text data definitions (`pokemon.txt`, `moves.txt`, `abilities.txt`, `items.txt`, `trainers.txt`, `encounters.txt`, `metadata.txt`, etc.). `Scripts/021_Compiler` parses these into binary `Data/*.dat` files, which are loaded at runtime into `GameData::*` classes defined in `Scripts/010_Data/002_PBS data/` (one file per data type: `Species`, `Move`, `Item`, `Ability`, `Trainer`, ...).

- Each `GameData::*` class mixes in `GameData::ClassMethods`/`ClassMethodsSymbols`/`ClassMethodsIDNumbers` (`Scripts/010_Data/001_GameData.rb`), giving uniform `.get`, `.try_get`, `.exists?`, `.each`, `.load`, `.save` accessors backed by a `DATA` hash.
...
There is no automated test suite. Verification means launching `Game.exe` in debug mode and exercising the relevant menu/battle/map flow directly (the F9 debug menu gives access to data editors, map/event tools, and a "fully compile all data" option useful after PBS edits). If `rubocop` is installed, `rubocop` can be run against changed files to catch style issues consistent with `.rubocop.yml`. See the `verifying-essentials-changes` skill before claiming any change works.

## Claude configuration in this repo

Repo-local skills live in `.claude/skills/`; personal hookify automation rules live as gitignored `.claude/hookify.*.local.md` files (see `.gitignore`). Start with the `pokemon-essentials-orchestration` skill when a task's scope isn't obvious — it maps tasks to the right skill(s). The `claude-repository-adaptation` skill keeps this configuration in sync as the repo (which is expected to keep restructuring) evolves.
