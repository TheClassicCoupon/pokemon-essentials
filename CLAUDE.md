...
002_Save data
003_Game processing     Core game loop, scene stack
004_Game classes
005_Sprites
006_Map renderer
007_Objects and windows UI framework primitives (windows, sprites, controls)
008_Audio
009_Scenes
...
`PBS/` contains human-editable, plain-text data definitions (`pokemon.txt`, `moves.txt`, `abilities.txt`, `items.txt`, `trainers.txt`, `encounters.txt`, `metadata.txt`, etc.). `Data/Scripts/021_Compiler` parses these into binary `Data/*.dat` files, which are loaded at runtime into `GameData::*` classes defined in `Data/Scripts/010_Data/002_PBS data/` (one file per data type: `Species`, `Move`, `Item`, `Ability`, `Trainer`, `Encounter`, ...).

- Each `GameData::*` class mixes in `GameData::ClassMethods`/`ClassMethodsSymbols` (`Data/Scripts/010_Data/001_GameData.rb`), giving uniform `.get`, `.try_get`, `.exists?`, `.each`, `.load`, `.save` accessors backed by a `DATA` hash.
...
- Compiler-side PBS parsing lives in `Data/Scripts/021_Compiler/002_Compiler_CompilePBS.rb` (PBS → Data) and `003_Compiler_WritePBS.rb` (Data → PBS, used by in-game data editors).

## Plugins

`Plugins/` (gitignored) holds third-party/optional plugin scripts, each in its own subfolder with a `meta.txt` describing name/version/dependencies/conflicts. `Data/Scripts/001_Technical/005_PluginManager.rb` reads `meta.txt` files, resolves load order from dependencies, compiles plugin scripts into `Data/PluginScripts.rxdata`, and evals them at boot. See the large comment block at the top of that file for the `meta.txt` format and `PluginManager.register` options.
